...
| الإدارة | مكتمل جزئيًا | 7 تبويبات؛ مالية (ميزان+مصروفات)؛ تدقيق؛ باقات وتسعير ونزاعات ومدفوعات من اللوحة |
...
الجرد: **18 منتهية / 1 شريحة أولى (M6) / 5 متبقية حيّة**. التفاصيل: `SPEC_BASELINE.md`.

آخر دفع على `origin/main`: **`e06bf79`**. طُبّق `020_general_ledger.sql` على Postgres المحلي.
...
| 4 | [~] | M3/M5 | e2e حي: المسار مؤكد من المشغّل؛ لقطة خطأ إن فشل |
| 5 | [~] | M6 | شريحة قيد مزدوج أولى (ترحيل + ميزان + قيود). ينقص فترات/مراكز تكلفة/تسوية بوابة |
| 6 | [~] | M7 | Twilio مؤجّل — لا يوقف الرفع |
| 7 | [x] | M8 | تبويبات الأدمن: لوحة / عمليات / أطراف / كتالوج / مالية / تحليلات / إدارة |
...
| 12 | [~] | M4 APK | v1.0.3 Finished (`bae7ee22`) — OSM/WebView؛ APK محلي `artifacts/mobile/dist/fanni.apk` (~96.8MB)؛ WinSCP → `/var/www/fanni-web/fanni.apk` |
...
| APK على `app.upnexa-eg.com` | قديم — WinSCP → `/var/www/fanni-web/fanni.apk` ثم `chmod 644` |
...
|---|---|---|---|
| 26 أغسطس 2026 | APK v1.0.3 EAS Finished | إصلاح `react-native-webview`/`expo config` (بلجن محلي)؛ فشلان `08c6ea00`/`51a8992e`؛ بناء ناجح `bae7ee22`؛ APK `artifacts/mobile/dist/fanni.apk` (~96.8MB) | WinSCP → `/var/www/fanni-web/fanni.apk`؛ تثبيت من الرابط؛ اختبار تسجيل+عنوان OSM |
