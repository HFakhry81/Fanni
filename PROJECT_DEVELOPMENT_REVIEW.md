...
اقتصاد المنتج المعتمد (`SPEC_BASELINE.md`): هدية ترحيب **60** مرة واحدة؛ افتراضي Lead **20**؛ الشحن **120 نقطة لكل 100 جنيه** (باقات 100/200/300/400/500 → 120/240/360/480/600). الخصم على السيرفر داخل معاملة واحدة مع دفتر محفظة وأوعية ترويجي/مشترى.
...
|---|---|---|
| API Server | مكتمل جزئيًا | محفظة بأوعية، Lead ذري، جيوفنس، تسعير Lead، اتصال مقنّع، فواتير بلا OCR، تدقيق مالي، CORS وحد معدّل |
| تطبيق الهاتف | مكتمل جزئيًا | حوار القبول، تتبع نعم/لا، نتيجة الخدمة، Lead Pricing، شروط استخدام، اتصال مقنّع |
| قاعدة البيانات | مكتمل جزئيًا | 017/018 اقتصاد وأوعية؛ `order_declines`؛ `masked_call_sessions`؛ `admin_audit_logs`؛ أعمدة OCR تاريخية |
| الإدارة | مكتمل جزئيًا | باقات، تسعير Lead، نزاعات، `GET /admin/audit-logs`؛ ينقص IA سبعة أقسام وشاشة تدقيق |
...
- نقاط داخل المنصة؛ باقات معتمدة 100→120 … 500→600 (migrations 017/018)؛ تكلفة افتراضية 20 في `unlock_costs`.
...
- اقتصاد معتمد في migrations **017/018**: ترحيب 60، Lead 20، باقات 100→120 … 500→600، أوعية ترويجي/مشترى.
- شروط الاستخدام في التسجيل؛ رفع خاص بلا `makePublic`؛ اتصال مقنّع في الكود؛ شحن يدوي حتى OPay.
- آخر دفع: `e06bf79`. قائمة المتبقي في القسم التالي (11 بندًا).

## يحتاج استكمالًا (أولوية) — قائمة مهام للاستئناف

الجرد: **13 منتهية / 11 متبقية** من 24 بند حوكمة ونظافة. التفاصيل: `SPEC_BASELINE.md`.

آخر دفع على `origin/main`: **`e06bf79`**. قبل الاختبار الحي:

`pnpm --filter @workspace/db run migrate`

لا تنتظر OPay أو باسورد الـ VPS إلا عند البندين 9 و10. **P0 قبل النشر: البند 2 (KYC).**

| # | الحالة | الوحدة | المهمة |
|---|---|---|---|
| 1 | [x] | M4 تشغيل | migrate 017/018/019 على بيئة التطوير (طُبّق 019 هنا) |
| 2 | [~] | M1 | KYC: `approval_status` عند رفع البطاقة (pending_review). مخزن VPS ينتظر بيانات السيرفر |
| 3 | [x] | M3 | نزاع رقم خاطئ / عدم رد آلي بلا نقر اتصال + حد يومي (`DISPUTE_AUTO_DAILY_CAP`) |
| 4 | [ ] | M3/M5 | اختبار e2e حي: عميل → فني → جيوفنس |
| 5 | [~] | M6 | شاشة مصروفات موجودة؛ ينقص قيد مزدوج كامل |
| 6 | [ ] | M7 | تشغيل Twilio في البيئة |
| 7 | [ ] | M8 | إعادة تجميع لوحة الأدمن (7 أقسام) |
| 8 | [x] | M0 | شاشة تدقيق في الأدمن (`/(admin)/(tabs)/audit-logs`) |
| 9 | [ ] | M4 | ربط OPay عند وصول الـ API |
| 10 | [ ] | M9 | تخزين الصور على VPS الإنتاجي |
| 11 | [x] | M2 | Accuracy/Source على `/geo/update` وتسجيل/تحديث الموقع |
